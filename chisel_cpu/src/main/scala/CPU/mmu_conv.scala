...
class mmu_conv extends Module{
  ...
  for (i <- 0 until 16){
    switch(io.vm_i){
      is(d.CSR_mstatus_vm_Mbare){
        hit(i) := false.B
      }

      is(d.CSR_mstatus_vm_Sv32){// ? 32bit vir_addr_i & 1bit tlb_mask(i)
  ...
  tlb_miss_exception := Mux(hit(0).orR === 1.U, false.B ,true.B)
  ...
  when(io.rst_n === d.RstEnable){
    ...
    when(tlb_miss_exception.asBool){
      ...
      io.tlb_update_o := update_exception(io.hit_index_o).asBool
    }
